...
CustomEvents.prototype._safeEvent = function(eventName) {
    let events = this.events; // eslint-disable-line
    let byName;
...
CustomEvents.prototype._memorizeContext = function(ctx) {
    ...
    const context = this._safeContext();
    const index = this._indexOfContext(ctx);
...
CustomEvents.prototype._forgetContext = function(ctx) {
    ...
    const context = this._safeContext();
    const contextIndex = this._indexOfContext(ctx);
...
CustomEvents.prototype.on = function(eventName, handler, context) {
    ...
    if (snippet.isString(eventName)) {
        ...
        snippet.forEach(eventName, name => {
            self._bindEvent(name, handler, context);
        });
    } else if (snippet.isObject(eventName)) {
        // [syntax 3, 4]
        context = handler;
        snippet.forEach(eventName, (func, name) => {
...
CustomEvents.prototype._offByEventName = function(eventName, handler) {
    const self = this;
    const forEach = snippet.forEachArray;
    const andByHandler = snippet.isFunction(handler);
    const matchHandler = self._matchHandler(handler);

    eventName = eventName.split(R_EVENTNAME_SPLIT);

    forEach(eventName, name => {
        const handlerItems = self._safeEvent(name);
...
CustomEvents.prototype._offByHandler = function(handler) {
    ...
    snippet.forEach(this._safeEvent(), handlerItems => {
        self._spliceMatches(handlerItems, matchHandler);
    });
};
